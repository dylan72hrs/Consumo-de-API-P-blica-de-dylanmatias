...
while True:                                                 # permite escribir al usuario las direcciones donde quieres ir
    print("\n+++++++++++++++++++++++++++++++++++++++++++++")
    print("Vehicle profiles available on Graphhopper:")          # permite QUE EL USUARIO PUEDA DECIDIR EN QUE  IRA A SU DESTINO SI EN BICI,CAMINANDO,AUTO ETC
    print("+++++++++++++++++++++++++++++++++++++++++++++")
    print("car, bike, foot")
    print("+++++++++++++++++++++++++++++++++++++++++++++")
    profile=["car", "bike", "foot"]                              # permite QUE EL USUARIO PUEDA DECIDIR EN QUE  IRA A SU DESTINO SI EN BICI,CAMINANDO,AUTO ETC
    ...
    if orig[0] == 200 and dest[0] == 200:                      
        ...
        paths_url = route_url + urllib.parse.urlencode({"key":key, "vehicle":vehicle}) + op + dp    # Si no se encuentra ninguna coincidencia, el car se asigna al vehicle 
        paths_status = requests.get(paths_url).status_code
        paths_data = requests.get(paths_url).json()
        print("Routing API Status: " + str(paths_status) + "\nRouting API URL:\n" + paths_url)  # permite que MUESTRE que APÍ STATUS  SEA CORRECTO Y QUE LLEGA AL IGUAL QUE LA URL
        print("=================================================")
        print("Directions from " + orig[3] + " to " + dest[3] + " by " + vehicle)         # permite que MUESTRE LA DURACION Y DISTANCIA DEL VIAJE
        print("=================================================")
        if paths_status == 200:
            ...
            for each in range(len(paths_data["paths"][0]["instructions"])):
                path = paths_data["paths"][0]["instructions"][each]["text"]                                   #este FOR convierte metros a MILLAS
                distance = paths_data["paths"][0]["instructions"][each]["distance"]
                print("{0} ( {1:.1f} km / {2:.1f} miles )".format(path, distance/1000, distance/1000/1.61))
            print("=============================================")                                    
        else:                                                                      # Este ELSE informa o muestra el error de que china no puede llegar a washinton como que dice que no ahi calle por eso po
